 ...
 * @date 20 April 2020
 * @version 1.0.1
 * @brief Hardcodes configs and uses Gurobi to solve a ETOL
 * @section DESCRIPTION
 ...
 *
 * The optimized trajectory is saved to a CSV file and are plotted with Gnuplot.
 ******************************************************************************/
...
#endif
#define XFILENAME       "xtraj.csv"
#define UFILENAME       "utraj.csv"
#define BIGM            1000
#define NSIDES          4
...
int main() {
    ...
    // X = {x,y}
    t->setXvartype({ETOL::var_t::CONTINUOUS, ETOL::var_t::CONTINUOUS});
    t->setXlower({0., 0.});
    t->setXupper({7., 7.});
    t->setX0({1., 2.});
    t->setXf({5., 4.});
    ...
    // U = {xdot, ydot}
    t->setUvartype({ETOL::var_t::CONTINUOUS, ETOL::var_t::CONTINUOUS,
        ETOL::var_t::CONTINUOUS, ETOL::var_t::CONTINUOUS});
    t->setUlower({-0.5, -0.5, -0.5, -0.5});
    t->setUupper({0.5, 0.5, 0.5, 0.5});
    ...
    // Set Dynamics (aka Difference equations))
    t->setXrhorizon(1);
    t->setUrhorizon(0);
    ...
    // Create Constraints
    ETOL::f_t abs0, obs0, saa0;  // Absolute value constraints

    // Absolute value constraint
    abs0 = &absConstraint;

    // Obstacle Constraint
    // Create  obstacle (supports 2D only atm)
    // Exclusion Zone is defined by corners of a polygon
    std::vector<ETOL::border_t> exz = {
            // Exclusion Zone 1
            {{3.2, 2.5, 0.},
            {3.4, 2.6, 0.},
            {3.5, 3.4, 0.},
    ...
    // Add constraints via Lambda expression
    for_each(exz.begin(), exz.end(), [t](ETOL::border_t &border){
            t->addExclZone(&border);
        });
    obs0 = obsConstraint(t);

    // Moving Exclusion Zone is defined by radius and center of a closed a ball
    std::vector<ETOL::track_t> mexz = {   // Moving Exclusion Zone 1
        // radius
        {.5,    {{  // t   // state
                    0.0, {2.0, 2.}},
                    {t->getDt()*t->getNSteps(), {2.5, 2.}}
                }
        },
        // Moving Exclusion Zone 2
        {.5,    {  // t   // state
                    {0.0, {1.0, 4.}},
                    {t->getDt()*t->getNSteps(), {1., 3.}}
                }
        }
    };
    for_each(mexz.begin(), mexz.end(), [t](ETOL::track_t &track){
        t->addAdjTrack(&track);
    });

    saa0 = saaConstraint(t);

    // Path Constraints
    t->setConstraints({&abs0, &obs0, &saa0});
    ...
    // Save
    std::cout << "State history saved to " <<
            ETOL::TrajectoryOptimizer::save(t->getXtraj(), XFILENAME) <<
            std::endl << "Control history saved to " <<
            ETOL::TrajectoryOptimizer::save(t->getUtraj(), UFILENAME) <<
            std::endl;
    ...
    ETOL::TrajectoryOptimizer::plotXY_wExclZones(t->getXtraj(),
                                                    t->getObstacles());
    ETOL::TrajectoryOptimizer::plotXY(t->getXtraj());
    ETOL::TrajectoryOptimizer::plot(t->getXtraj());
    ETOL::TrajectoryOptimizer::plot(t->getUtraj());
