...
ETOL was originally developed in Ubuntu 18.04. However, with `MSYS2`_, ETOL and its prerequisite software packages can be installed on Windows because MSYS2 provides a Unix-like environment for Windows.

.. _MSYS2 : https://www.msys2.org/

The MSYS2 shell uses a package manager called pacman. This `MSYS2 package manager`_ is able to install the prerequisite software packages for ETOL.  First install, the default build tool ::

  pacman -S binutils diffutils git grep make patch pkg-config

The mingw64 build tools should be installed with ::

  pacman -S mingw-w64-x86_64-toolchain

When prompted, select all. After the install completes, the following command block installs additional dependencies ::
...
Since the Python package, sphinx-git, is not available through pacman, pip should be used to install it ::
